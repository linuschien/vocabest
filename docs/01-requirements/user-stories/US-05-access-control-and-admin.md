...
### 驗收條件 (Acceptance Criteria)
- **AC1**：系統管理員可以檢視、新增、修改、刪除字庫 (WordBank) 與靜態題庫 (StaticQuestionBank) 的所有內容。由於資料數量龐大，列表檢視頁面必須實作分頁 (Pagination) 機制。
- **AC2**：系統管理員可以檢視全站所有使用者 (User) 的資料與學習進度，進行總體數據的追蹤。
- **AC3**：一般使用者若嘗試呼叫管理員專屬 API 端點 (如新增字庫、修改題庫等)，系統須阻擋請求並回傳 HTTP `403 Forbidden`。
- **AC4**：系統預設應將 `linus.chien@gmail.com` 綁定為系統管理員 (Admin) 角色。
- **AC5 (觸發資料管線)**：系統管理員介面應提供明確的操作入口，允許管理員手動觸發「官方詞彙表匯入 (US-00-03)」與「AI 批次生成靜態題庫 (US-00-04)」，並監控這些非同步背景任務的執行狀態。
